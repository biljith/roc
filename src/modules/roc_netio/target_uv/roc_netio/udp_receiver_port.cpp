...
bool UDPReceiverPort::join_multicast_group_() {
    ...
    if (int err = uv_udp_set_membership(&handle_, packet::ip_to_str(address_).c_str(),
                                        address_.miface(), UV_JOIN_GROUP)) {
        roc_log(LogError, "udp receiver: uv_udp_set_membership(): [%s] %s",
                uv_err_name(err), uv_strerror(err));
        return false;
    }

    roc_log(LogDebug, "udp receiver: join multicast group: address=%s interface=%s",
            packet::address_to_str(address_).c_str(), address_.miface());
...
void UDPReceiverPort::leave_multicast_group_() {
    ...
    if (int err = uv_udp_set_membership(&handle_, packet::ip_to_str(address_).c_str(),
                                        address_.miface(), UV_LEAVE_GROUP)) {
        roc_log(LogError, "udp receiver: uv_udp_set_membership(): [%s] %s",
                uv_err_name(err), uv_strerror(err));
    }

    roc_log(LogDebug, "udp receiver: leave multicast group: address=%s interface=%s",
            packet::address_to_str(address_).c_str(), address_.miface());
}
